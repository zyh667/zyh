...
<div class="top">
		...
		<div class="register-view">
            <h3>{{ message }}</h3>
				...
				<div class="btn2"><input type="submit" @click="modify" class="button" value="修改密码"></div>
		</div>
</template>
<script>
...
export default {
  ...
  methods:{
    ...
    async modify(){
      try {
        const response = await axios.post('/index/forget_password/',{
          phone:this.phone,
          code:this.code
        });
        const data=response.data
        if (data.status === "success") {
           this.$router.push({name:'modify_pwd'})
         }
         else {
